        ...
        <nav class="flex flex-1 flex-col gap-y-6">
            ...
            <div>
                ...
                <ul role="list" class="space-y-1">
                    ...
                    <!-- Team Notifications (Team plan only) -->
                    @if(in_array($billingPlan, ['team', 'business'], true) && $currentTeam)
                    <li>
                        <a href="{{ route('team.notifications', $currentTeam->id) }}" class="{{ request()->routeIs('team.*') ? 'bg-emerald-50 text-emerald-700 font-semibold' : 'text-gray-700 hover:bg-gray-50 hover:text-emerald-700' }} group flex items-center gap-x-3 rounded-lg px-3 py-2.5 text-sm transition-all">
